...
export default function DateNavigation({ dates = [], currentDate = 'Hoje', onPrevious, onNext }) {
  ...
  const handlePrevious = () => {
    if (hasPrevious && onPrevious) {
      onPrevious();
    } else if (hasPrevious && currentIndex > 0) {
      // Fallback: navegação automática se não houver callback
      const prevDate = datesToShow[currentIndex - 1];
      // Aqui você pode atualizar o estado ou disparar um evento
    }
  };
  ...
  const handleNext = () => {
    if (hasNext && onNext) {
      onNext();
    } else if (hasNext && currentIndex < datesToShow.length - 1) {
      // Fallback: navegação automática se não houver callback
      const nextDate = datesToShow[currentIndex + 1];
      // Aqui você pode atualizar o estado ou disparar um evento
    }
  };

  return (
    <nav className="flex items-center justify-between mb-4 bg-white rounded-lg p-2 sm:p-3 overflow-x-auto">
